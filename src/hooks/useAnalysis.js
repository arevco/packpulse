...
export function useAnalysis({ mappingConfirmed, allUploaded, inventory, itemMaster, boms, workOrders, productionData, invMapping, bomMapping, woMapping, poData, poMapping, edrData, edrTimestamp, dockData, dockTimestamp }) {
  ...
  var supplyRiskModel = useMemo(function() {
    return buildSupplyRiskModel({
      analysis: analysis,
  ...
  var inboundCoverage = supplyRiskModel ? supplyRiskModel.inboundCoverage : null;
  ...
  var recommendationBundle = useMemo(() => {
    ...
    if (!edrData || !edrData.length) {
      recs.push({
        ...
        priorityScore: 55,
        owner: "Ops Analyst",
        action: "Sync Receive Orders",
        why: "Inbound coverage confidence is reduced without Receive Orders data.",
        impactUnits: 0,
        window: "today",
        confidence: "Low",
        source: "Data Quality",
